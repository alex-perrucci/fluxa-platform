...
export function CatalogConsole({
  ...
  async function refresh(preferredPriceListId = priceList?.id ?? '') {
    const [nextCategories, nextVatRates, nextProducts, nextPriceLists] =
      await Promise.all([
        requestJson<CatalogPage<CatalogCategory>>(
          '/api/control-center/merchant/configuration/categories?page=1&pageSize=100',
        ),
        requestJson<CatalogPage<VatRate>>(
          '/api/control-center/merchant/configuration/vat-rates?page=1&pageSize=100',
        ),
        requestJson<CatalogPage<CatalogProduct>>(
          '/api/control-center/merchant/configuration/products?page=1&pageSize=100',
        ),
        requestJson<CatalogPage<PriceList>>(
          '/api/control-center/merchant/configuration/price-lists?page=1&pageSize=100',
        ),
      ]);
  ...
  async function createProduct(event: FormEvent<HTMLFormElement>) {
    ...
    await run(async () => {
      ...
      const product = await requestJson<CatalogProduct>(
        '/api/control-center/merchant/configuration/products',
        {
          method: 'POST',
      ...
      try {
        await requestJson(
          `/api/control-center/merchant/configuration/price-lists/${priceList.id}/prices`,
          {
            method: 'PUT',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({
              productId: product.id,
              amountCents: centsFromInput(form.get('price')),
            }),
          },
        );
      } catch (priceError) {
  ...
  async function saveProduct(
    ...
    await run(async () => {
      await requestJson(
        `/api/control-center/merchant/configuration/products/${product.id}`,
        {
          method: 'PATCH',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
  ...
  async function saveCategory(
    ...
    await run(async () => {
      await requestJson(
        `/api/control-center/merchant/configuration/categories/${category.id}`,
        {
          method: 'PATCH',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
      ...
      await requestJson('/api/control-center/merchant/configuration/vat-rates', {
        ...
        body: JSON.stringify({
          ...
          rateBasisPoints: Math.round(
            Number(String(form.get('rate') ?? '0').replace(',', '.')) * 100,
          ),
          natureCode: String(form.get('natureCode') ?? '').trim() || undefined,
  ...
  async function saveVat(event: FormEvent<HTMLFormElement>, vat: VatRate) {
    event.preventDefault();
    const form = new FormData(event.currentTarget);
    const natureCode = String(form.get('natureCode') ?? '').trim();
    await run(async () => {
      await requestJson(
        `/api/control-center/merchant/configuration/vat-rates/${vat.id}`,
        {
          method: 'PATCH',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
  ...
  async function createPriceList(event: FormEvent<HTMLFormElement>) {
    ...
    const form = new FormData(formElement);
    await run(async () => {
      const created = await requestJson<PriceList>(
        '/api/control-center/merchant/configuration/price-lists',
        {
          method: 'POST',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
      ...
      if (locationId) {
        await requestJson(
          `/api/control-center/merchant/configuration/price-lists/${created.id}/locations`,
          {
            method: 'PUT',
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({
  ...
  async function savePriceList(
    ...
    await run(async () => {
      await requestJson(
        `/api/control-center/merchant/configuration/price-lists/${item.id}`,
        {
          method: 'PATCH',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
  ...
  async function assignCurrentPriceList() {
    ...
    await run(async () => {
      await requestJson(
        `/api/control-center/merchant/configuration/price-lists/${priceList.id}/locations`,
        {
          method: 'PUT',
          headers: { 'content-type': 'application/json' },
          body: JSON.stringify({
    ...
    <div>
        ...
        <section className="glass-panel panel-padding mt-5">
          <div className="wizard-actions">
            <div>
              ...
              <p className="muted">
                Il prezzo mostrato appartiene al listino selezionato.
              </p>
            </div>
            <select
              disabled={pending}
              onChange={(event) => void loadPriceList(event.target.value)}
              value={priceList?.id ?? ''}
            >
          ...
          {canManage ? (
            <form className="form-grid mt-5" onSubmit={createProduct}>
              ...
              <label className="field">
                <span>IVA</span>
                <select name="vatRateId" required>
                  {activeVatRates.map((item) => (
                    <option key={item.id} value={item.id}>
                      {item.name}
                    </option>
                  ))}
              ...
              <label className="field">
                <span>Unità</span>
                <select name="unit">
                  <option value="EACH">Pezzo</option>
              ...
              <label className="field">
                ...
                <span>
                  <input name="trackAvailability" type="checkbox" /> Traccia
                  disponibilità
                </span>
              </label>
          ...
          <div className="data-list mt-5">
            {products.map((product) => (
              <form
                className="data-row"
                key={`${product.id}:${priceList?.id ?? 'none'}`}
              ...
              >
                <div>
                  ...
                  />
                  <small>
                    {product.categoryName} · IVA {product.vatRateBasisPoints / 100}%
                  </small>
                </div>
                <div>
                  <input
                ...
                <div>
                  ...
                  >
                    {categories.map((item) => {
                      ...
                      return (
                        <option
                          disabled={inactive && item.id !== product.categoryId}
                          key={item.id}
                          value={item.id}
                        >
                          {item.name}
                  ...
                  <select
                    defaultValue={product.vatRateId}
                    disabled={!canManage}
                    name="vatRateId"
                  >
                    {vatRates.map((item) => {
                      ...
                      return (
                        <option
                          disabled={inactive && item.id !== product.vatRateId}
                          key={item.id}
                          value={item.id}
                        >
                          {item.name}
                ...
                <div>
                  <input
                    defaultValue={(
                      (prices.get(product.id)?.amountCents ?? 0) / 100
                    ).toFixed(2)}
                    disabled={!canManage || !priceList}
                    min="0"
                    name="price"
                    step="0.01"
                    type="number"
                  />
                  <small>{euro(prices.get(product.id)?.amountCents)}</small>
                </div>
                <div>
                  <select
      ...
      {tab === 'categories' ? (
        <section className="glass-panel panel-padding mt-5">
          ...
          {canManage ? (
            <form className="form-grid mt-5" onSubmit={createCategory}>
              <label className="field">
                <span>Codice</span>
                <input name="code" required />
              </label>
              ...
              <label className="field">
                <span>Colore</span>
                <input name="colorHex" placeholder="#2563EB" />
              </label>
              <label className="field">
                <span>Ordine</span>
          ...
          <div className="data-list mt-5">
            {categories.map((item) => (
              <form
                className="data-row"
                key={item.id}
                onSubmit={(event) => void saveCategory(event, item)}
              >
                <div>
                  <input
      ...
      {tab === 'vat' ? (
        <section className="glass-panel panel-padding mt-5">
          ...
          {canManage ? (
            <form className="form-grid mt-5" onSubmit={createVat}>
              <label className="field">
                <span>Codice</span>
                <input name="code" required />
              </label>
              ...
              <label className="field">
                <span>Aliquota %</span>
                <input max="100" min="0" name="rate" required step="0.01" type="number" />
              </label>
              <label className="field">
                <span>Natura (se 0%)</span>
                <input name="natureCode" placeholder="N2.2" />
              </label>
              <label className="field">
          ...
          <div className="data-list mt-5">
            {vatRates.map((item) => (
              <form
                className="data-row"
                key={item.id}
                onSubmit={(event) => void saveVat(event, item)}
              >
                <div>
                  <input
                    defaultValue={item.name}
                    ...
                    required
                  />
                  <small>{item.isDefault ? 'Predefinita' : item.code}</small>
                </div>
                <input
                  defaultValue={item.code}
                  disabled={!canManage}
        ...
        <section className="glass-panel panel-padding mt-5">
          <div className="wizard-actions">
            ...
            <div className="filter-bar">
              ...
              <button
                className="button-secondary"
                disabled={!canManage || !priceList || !locationId || pending}
                onClick={() => void assignCurrentPriceList()}
                type="button"
              >
                Assegna listino selezionato
              </button>
            </div>
          </div>
          {canManage ? (
            <form className="form-grid mt-5" onSubmit={createPriceList}>
              <label className="field">
                <span>Codice</span>
                <input name="code" required />
              </label>
          ...
          <div className="data-list mt-5">
            {priceLists.map((item) => (
              <form
                className="data-row"
                key={item.id}
                onSubmit={(event) => void savePriceList(event, item)}
              >
                <div>
                  <input
                    defaultValue={item.name}
